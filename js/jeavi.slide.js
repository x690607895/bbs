...
function funcmaincontent(){
	...
	function startslide(){
		...
		for(var i = 0 ; i < _maxpage ; i++){
			var _pos = Math.round(1190*(i-_currentpage)+jQuery(window).width()/2-595);
			var _opa = 0.5;
		...
		rolbtn = setInterval(nextPage,4595);//Execution time 4595
		jQuery("#maincontent").hover(
			function(){clearInterval(rolbtn)},function(){rolbtn = setInterval(nextPage,4595)}//Execution time 4595
		    );
	 ...
	 prevPage = function(){
		...
		for(var i = 0 ; i < _maxpage ; i++){
			...
			if(_pos < -1190){
				_pos += _maxpage*1190
			}else if(_pos > jQuery(window).width()+1190){
				_pos -= _maxpage*1190
			}
